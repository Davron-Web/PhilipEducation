...
class IeltsListeningController extends Controller
{
    ...
    {
        $passages = IeltsPassage::where('skill', 'listening')->withCount('questions')->orderBy('id')->get();
        ...
        return view('public.ielts.listening.index', compact('passages', 'bestScores'));
    }

    public function show(IeltsPassage $passage): View
    {
        abort_unless($passage->skill === 'listening', 404);

        $lastAttempt = $passage->attemptsFor(Auth::user())->first();

        return view('public.ielts.listening.show', compact('passage', 'lastAttempt'));
    }
    ...
    public function submit(Request $request, IeltsPassage $passage): RedirectResponse
    {
        ...
        return redirect()->route('ielts.listening.show', $passage)
