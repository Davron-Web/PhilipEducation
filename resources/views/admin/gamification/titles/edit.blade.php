...
@section('title', 'Изменить титул')

@section('content')
    <x-admin.page-header title="Изменить титул" :backRoute="route('admin.gamification.titles.index')" />

    <form action="{{ route('admin.gamification.titles.update', $title) }}" method="POST" class="card">
        @csrf @method('PUT')
        <div class="form-grid">
